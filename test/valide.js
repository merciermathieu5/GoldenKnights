...
(async () => {
  ...
  egal(S.SECOURS_ROSTER.length, 26, '26 joueurs dans la formation de secours (23 patineurs + 3 gardiens)');
  const eic = S.SECOURS_ROSTER.find(j => j.nom === 'Jack Eichel');
  egal(eic.salaire, 6000000, 'Salaire Eichel (page du 25 juin : 6 000 000 $)');
  egal(eic.ov, 81, 'Eichel OV 81 (cote réelle de la page, pas la vraisemblance NHL)');
  egal(eic.ct, 2, 'Contrat Eichel 2 ans');
  const mar = S.SECOURS_ROSTER.find(j => j.nom === 'Mitch Marner');
  egal(mar.salaire, 8500000, 'Salaire Marner');
  egal(mar.ov, 83, 'Marner OV 83');
  egal(S.SECOURS_Y21.length, 23, '23 patineurs dans la référence Y21 (capture XtraStats du DG, validés par ratios)');
  const eicY21 = S.SECOURS_Y21.find(x => x.nom === 'Jack Eichel');
  egal(eicY21.pts, 78, 'Eichel 78 points en Y21');
  egal(eicY21.shots, 228, 'Eichel 228 tirs en Y21 (confirmé par Sh/G 2,78 et Sh/20 2,50)');
  const braY21 = S.SECOURS_Y21.find(x => x.nom === 'Jakub Brabenec');
  egal(braY21.gp, 4, 'Brabenec : quatre matchs en Y21 (transcrit tel quel)');
  ...
  egal(profilDe('Mitch Marner').profil, 'Elite', 'Marner → Elite (le seul du club)');
  tableauEgal(profilDe('Mitch Marner').stats, ['shotpct','gwg','ppg','pts','pmrang'],
    'Stats évaluées Elite = tableau 20 (PCTG, GWG, PP, P, +/-)');
  egal(profilDe('Jack Eichel').profil, 'Playmaker', 'Eichel → Playmaker');
  tableauEgal(profilDe('Jack Eichel').stats, ['assists','pts'], 'Stats Playmaker = A, P');
  egal(profilDe('Mark Stone').profil, 'Playmaker', 'Stone → Playmaker');
  egal(profilDe('Tomas Hertl').profil, 'Sniper', 'Hertl → Sniper');
  egal(profilDe('Tyler Seguin').profil, 'Sniper', 'Seguin → Sniper');
  egal(profilDe('Troy Terry').profil, 'Sniper', 'Terry → Sniper');
  egal(profilDe('Brett Howden').profil, 'Power Forward', 'Howden → Power Forward');
  egal(profilDe('Reilly Smith').profil, 'Power Forward', 'Smith → Power Forward');
  egal(profilDe('Keegan Kolesar').profil, 'Grinder', 'Kolesar → Grinder');
  tableauEgal(profilDe('Keegan Kolesar').stats, ['hits20'], 'Stats Grinder = MEÉ/20');
  egal(profilDe('Aatu Raty').profil, 'Prospect Playmaker', 'Raty (23 ans) → Prospect Playmaker');
  egal(profilDe('Zachary Bolduc').profil, 'Prospect Playmaker', 'Bolduc (22 ans) → Prospect Playmaker');
  egal(profilDe('Roby Jarventie').profil, 'Prospect Power Forward', 'Jarventie (23 ans) → Prospect Power Forward');
  egal(profilDe('Matyas Sapovaliv').profil, 'Prospect Two-Way Forward', 'Sapovaliv (21 ans) → Prospect Two-Way');
  egal(profilDe('Jakub Brabenec').profil, 'Prospect Depth Forward', 'Brabenec (22 ans) → Prospect Depth Forward');
  egal(profilDe('MacKenzie Weegar').profil, 'DDefensive', 'Weegar → DDefensive (seul défenseur vétéran)');
  egal(profilDe('Kaedan Korczak').profil, 'Prospect DEliteShutdown', 'Korczak (24 ans) → Prospect DEliteShutdown');
  egal(profilDe('Alex Vlasic').profil, 'Prospect DEliteShutdown', 'Vlasic (24 ans) → Prospect DEliteShutdown');
  egal(profilDe('Jamie Drysdale').profil, 'Prospect DEliteQB', 'Drysdale (23 ans) → Prospect DEliteQB');
  egal(profilDe('Lukas Cormier').profil, 'Prospect DEliteQB', 'Cormier (23 ans) → Prospect DEliteQB');
  egal(profilDe('Carter Yakemchuk').profil, 'Junior DEliteQB', 'Yakemchuk (20 ans) → Junior DEliteQB');
  egal(profilDe('Logan Thompson').profil, 'Backup Goalie', 'Thompson (OV 79) → Backup Goalie');
  egal(profilDe('Calle Clang').profil, 'Backup Goalie', 'Clang (OV 78) → Backup Goalie');
  egal(profilDe('Joel Hofer').profil, 'Backup Goalie', 'Hofer (OV 77) → Backup Goalie — aucun gardien OV 80+, aucun Starter (règle 6.2.5)');
  tableauEgal(profilDe('Logan Thompson').stats, ['mp','qggp','psv'], 'Stats Backup = MP, QG/GP, Psv');
  ...
  const jk = joueurDe('Mitch Marner');
  ...
  const fixtureXtra = [
    ...
    '    [Jack Eichel](https://x/#Jack Eichel) VEGAS           C   82  30  45  75 200  30 1700 100 2.44',
    '    [Pierre-Luc Dubois](https://x/#Pierre-Luc Dubois) ANAHEIM         C   76  30  38  68 199  23 1713 137 2.87',
    '    [* Backup_RW](https://x/#Backup_RW) VEGAS           LW   3   0   0   0   0   0    0   0 0.00'
  ].join('\n');
  const x = S.parseXtra(fixtureXtra, 'VEGAS');
  egal(x.length, 2, 'Deux patineurs VEGAS extraits (Anaheim exclu)');
  ...
  egal(rangees.length, 26, '26 rangées dans la table d\'alignement');
  ok(doc.querySelector('#alignSommaire').textContent.includes('Masse salariale'), 'Sommaire de masse salariale rendu');
  ok(doc.querySelector('#ficheEquipe').textContent.includes('VEGAS'), 'Fiche d\'équipe affichée');
  ...
  egal(masse, 71280000, 'Masse salariale des 26 pros = 71 280 000 $ (vérifiée contre la moyenne de la page : 2 741 538 $ × 26)');
  ok(!doc.querySelector('#alignSommaire .stat-carte').classList.contains('alerte'), 'Sous le plafond : aucune alerte');

  console.log('— Divers');
  egal(S.matchsEquipe(), 0, 'Fiche VEGAS 0-0-0 → 0 match d\'équipe');
  ...
  if (ETAT){
    ...
    ETAT.xtra = [{nom:'Jack Eichel', gp:9, goals:4, assists:6, pts:10, shots:25, pim:2, mp:180, hits:8}];
    const jEic = S.SECOURS_ROSTER.find(x2=>x2.nom==='Jack Eichel');
    const prodRepli = S.productionDe(jEic);
  ...
  const prodY21 = S.productionDe(S.SECOURS_ROSTER.find(x2=>x2.nom==='Jack Eichel'));
  egal(prodY21?._modeY21, true, 'Production servie par les données Y21 intégrées');
  egal(prodY21?.pts, 78, 'Points Y21 d\'Eichel');
  const carteEic = [...doc.querySelectorAll('#progGrille .joueur-carte')]
    .find(c=>c.querySelector('.jc-nom').textContent==='Jack Eichel');
  ok(!!carteEic, 'Carte d\'Eichel rendue en mode Y21');
  ok(carteEic.textContent.includes('Mode vérification Y21'), 'Note du mode Y21 sur la carte');
  const jEi = S.ETAT.roster.find(x2=>x2.nom==='Jack Eichel');
  const evPtsE = S.evaluerStat('pts', S.valeurStat(jEi, prodY21, 'pts', {}), S.seuilsPour(jEi, 'pts', {}, {}).seuils);
  egal(evPtsE.statut, 'memorable', 'Eichel Mémorable en points (78 > 73, Playmaker OV 81)');
  const jSt = S.ETAT.roster.find(x2=>x2.nom==='Mark Stone');
  const evPtsS = S.evaluerStat('pts', S.valeurStat(jSt, S.productionDe(jSt), 'pts', {}), S.seuilsPour(jSt, 'pts', {}, {}).seuils);
  egal(evPtsS.statut, 'correcte',
    'Stone Correcte en points : 67 = seuil Satisfaisante exactement (Playmaker OV 82), le dépassement STRICT s\'applique à chaque degré');
  const jMa = S.ETAT.roster.find(x2=>x2.nom==='Mitch Marner');
  const evPtsM = S.evaluerStat('pts', S.valeurStat(jMa, S.productionDe(jMa), 'pts', {}), S.seuilsPour(jMa, 'pts', {}, {}).seuils);
  egal(evPtsM.statut, 'decevante', 'Marner Décevante en points (37 < 55 ≤ 63, Elite OV 83) — les attentes Elite sont sévères');
  ok(!carteEic.textContent.includes('proj.'), 'Aucune projection affichée (saison complète)');
  btnY21.click();
